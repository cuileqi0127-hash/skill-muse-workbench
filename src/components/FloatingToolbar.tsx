...
export function FloatingToolbar({ onNewSession, onOpenHistory }: FloatingToolbarProps) {
  return (
    <div className="fixed left-4 top-1/2 z-30 flex -translate-y-1/2 flex-col gap-1.5 rounded-xl border border-border/60 bg-card p-1.5 shadow-sm">
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            onClick={onNewSession}
            className="flex h-8 w-8 items-center justify-center rounded-lg bg-accent text-accent-foreground transition-all hover:bg-primary hover:text-primary-foreground"
          >
